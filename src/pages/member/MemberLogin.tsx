...
export default function MemberLoginPage() {
  ...
  const signInWithGoogle = async () => {
    await supabase.auth.signInWithOAuth({ provider: "google", options: { redirectTo: `${window.location.origin}/member` } });
  };
      ...
      <div className="min-h-screen bg-gradient-to-br from-indigo-50 to-white dark:from-slate-950 dark:to-slate-900 flex items-center justify-center p-4">
        <div className="w-full max-w-sm">
          <div className="text-center mb-8">
            ...
            <p className="text-muted-foreground text-sm mt-1">Sign in to your church member account</p>
          </div>
          ...
          <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 p-6 space-y-4">
            ...
            <div className="relative">
              ...
              <div className="relative flex justify-center text-xs text-muted-foreground"><span className="bg-white dark:bg-slate-900 px-2">or</span></div>
            </div>

            <Button variant="outline" className="w-full h-11 rounded-full gap-2" onClick={signInWithGoogle}>
          ...
          <div className="text-center mt-6 space-y-2 text-sm text-muted-foreground">
            <p>Don't have an account? <Link to="/member/join" className="text-indigo-600 font-medium">Join a Church</Link></p>
            <p><Link to="/auth/signin" className="text-indigo-600">Admin Sign In</Link></p>
          </div>
        </div>
      </div>
    </>
