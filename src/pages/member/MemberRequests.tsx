...
import { HandHeart, Plus, MessageSquare, Heart, UserCheck, DollarSign, Stethoscope, Flower2, HelpCircle } from "lucide-react";

const TYPE_ICONS: Record<string, React.ElementType> = {
  prayer: Heart, counselling: UserCheck, visitation: UserCheck,
  financial_aid: DollarSign, medical_support: Stethoscope,
  bereavement: Flower2, general: HelpCircle,
};
    ...
    <>
      ...
      <div className="max-w-lg mx-auto space-y-4">
        <div className="flex items-center justify-between">
          ...
          <Button className="rounded-full gap-1.5" onClick={() => setSheet(true)}><Plus className="h-4 w-4" />Submit</Button>
        </div>
        ...
        {isLoading ? (
          ...
          <div className="text-center py-16 text-muted-foreground">
            ...
            <Button className="mt-4 rounded-full" onClick={() => setSheet(true)}><Plus className="h-4 w-4 mr-1" />Submit a Request</Button>
          </div>
        ) : (
          <div className="space-y-3">
            {requests.map((r: any) => {
              const Icon = TYPE_ICONS[r.request_type] || HelpCircle;
              return (
                <div key={r.id} className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-4 space-y-2">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        <Badge className={`text-xs capitalize ${STATUS_COLORS[r.status] || ""}`}>{r.status?.replace(/_/g, " ")}</Badge>
                      </div>
                  ...
                  {r.description && <p className="text-sm text-muted-foreground line-clamp-2 pl-13">{r.description}</p>}
                </div>
              );
            })}
          </div>
      ...
      <Sheet open={sheet} onOpenChange={setSheet}>
        <SheetContent className="overflow-y-auto">
          <SheetHeader><SheetTitle>Submit a Request</SheetTitle></SheetHeader>
          <div className="space-y-4 mt-6">
            <div className="space-y-1.5">
              ...
              <Select value={form.request_type} onValueChange={v => setForm(f => ({ ...f, request_type: v }))}>
                ...
                <SelectContent>
                  {["prayer", "counselling", "visitation", "financial_aid", "medical_support", "bereavement", "general"].map(t => (
