...
const MemberProfile = () => {
  ...
  const { data: member, isLoading } = useQuery({
    ...
    queryFn: async () => {
      // Try members table first (new flow), fall back to users for legacy staff accounts
      const { data: memberData } = await supabase.from("members").select("id, tenant_id, email, first_name, last_name, phone, date_of_birth, join_date, status, gender, avatar_url, member_type, created_at, updated_at").eq("id", memberId!).maybeSingle();
      if (memberData) return { ...memberData, role: memberData.member_type || "member" };
