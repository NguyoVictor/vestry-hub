...
-- Create a function to be called by cron job or trigger
CREATE OR REPLACE FUNCTION auto_update_trending_songs()
RETURNS VOID
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
BEGIN
  -- Update trending songs for all tenants
  FOR tenant_record IN SELECT id FROM tenants LOOP
    PERFORM update_trending_songs(tenant_record.id);
  END LOOP;
END;
$$;
...
COMMENT ON FUNCTION get_usage_reports IS 'Generate monthly or yearly usage reports with top songs and statistics';
COMMENT ON FUNCTION update_trending_songs IS 'Update trending status for songs based on recent usage patterns';
COMMENT ON FUNCTION get_song_recommendations IS 'Get smart song recommendations based on usage patterns and context';
COMMENT ON FUNCTION get_usage_analytics_summary IS 'Get comprehensive usage analytics summary for dashboard';
COMMENT ON FUNCTION auto_update_trending_songs IS 'Auto-update trending songs for all tenants (to be called by cron job)';
