...
// This function is also exported as a utility for other edge functions to import.
// It fetches the church's email_branding row and wraps content in the branded template.

export interface BrandedEmailOptions {
  tenantId: string;
  churchName: string;
  subject: string;
  bodyHtml: string;          // The specific email content (already HTML)
...
export async function buildBrandedEmail(opts: BrandedEmailOptions): Promise<string> {
  const { tenantId, churchName, bodyHtml, ctaLabel, ctaUrl, supabaseClient } = opts;

  // Fetch branding (non-fatal — fall back to defaults if not found)
  ...
  const footerText    = branding?.footer_text    ?? `${churchName} | You are receiving this because you are a member of ${churchName}.`;
...
// ─── HTTP handler (for direct invocation / testing) ───────────────────────────
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const html = await buildBrandedEmail({ ...body, supabaseClient: supabase });
